...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden transition-all ${open ? 'border-[#0D9488]/30 bg-[#ECFDF5]/40' : 'border-slate-200 bg-white'}`}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-5 py-4 text-left gap-4"
      >
        <span className={`text-sm font-semibold leading-snug ${open ? 'text-[#0D9488]' : 'text-slate-800'}`}>
          {question}
        </span>
        {open
          ? <ChevronUp className="w-4 h-4 text-[#0D9488] flex-shrink-0" />
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
      ...
      <div className="bg-[#0F172A] px-6 py-8 relative overflow-hidden">
        <div className="absolute -right-8 -top-8 w-40 h-40 bg-[#0D9488]/10 rounded-full blur-2xl pointer-events-none" />
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 bg-[#0D9488]/15 border border-[#0D9488]/25 rounded-full px-3 py-1 mb-4">
            <Zap className="w-3.5 h-3.5 text-[#0D9488]" />
            <span className="text-xs font-semibold text-[#0D9488]">{t('how.badge')}</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-6 relative z-10">
          {[
            ...
            <div key={label} className="bg-white/5 border border-white/10 rounded-2xl p-3 text-center">
              <Icon className="w-4 h-4 text-[#0D9488] mx-auto mb-1.5" />
      ...
      <div className="px-5 py-6 space-y-8">
        ...
        {tab === 'cliente' ? (
          <div className="space-y-6">
            ...
            <StepList steps={clientSteps} accent="#0D9488" />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
              ...
              <div className="space-y-2.5">
                ...
                <div className="w-full h-px bg-slate-200" />
                <div className="flex justify-between items-center bg-[#ECFDF5] -mx-1 px-3 py-2 rounded-lg">
                  <span className="text-sm font-bold text-slate-800">Cliente paga</span>
                  <span className="text-base font-extrabold text-[#0D9488]">{fmt(examplePrice)}</span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-[#0F172A] rounded-2xl p-5 text-center">
          ...
          <div className="flex items-center justify-center gap-1 text-[#0D9488] font-semibold text-sm">
