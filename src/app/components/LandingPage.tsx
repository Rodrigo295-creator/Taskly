...
const FEATURE_KEYS = [
  { icon: MessageCircle, title: 'landing.features.chat.title', desc: 'landing.features.chat.desc', accent: 'from-violet-500 to-purple-600' },
  { icon: Wallet, title: 'landing.features.pay.title', desc: 'landing.features.pay.desc', accent: 'from-emerald-500 to-teal-600' },
  { icon: Search, title: 'landing.features.track.title', desc: 'landing.features.track.desc', accent: 'from-sky-500 to-cyan-600' },
  { icon: Shield, title: 'landing.features.verify.title', desc: 'landing.features.verify.desc', accent: 'from-amber-500 to-orange-600' },
  { icon: LayoutDashboard, title: 'landing.features.dash.title', desc: 'landing.features.dash.desc', accent: 'from-rose-500 to-pink-600' },
  { icon: Star, title: 'landing.features.grow.title', desc: 'landing.features.grow.desc', accent: 'from-[#F97316] to-red-500' },
] as const;
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1e293b] overflow-hidden shadow-sm">
      ...
      >
        ...
        <ChevronDown
          className={`w-5 h-5 shrink-0 text-[#F97316] transition-transform ${open ? 'rotate-180' : ''}`}
        />
      </button>
      {open && (
...
export function LandingPage({ onSignIn, onStart }: Props) {
  ...
  const stats = [
    { value: '2.400+', label: t('landing.stats.verified'), color: 'text-[#F97316]' },
    { value: '40+', label: t('landing.stats.categories'), color: 'text-cyan-500' },
    { value: '100%', label: t('landing.stats.secure'), color: 'text-emerald-500' },
    { value: '4,8', label: t('landing.stats.rating'), color: 'text-violet-500' },
  ];

  const steps = [
    { icon: Search, n: '01', title: 'landing.how.step1.title', desc: 'landing.how.step1.desc', ring: 'ring-cyan-500/30 bg-cyan-500/10 text-cyan-600' },
    { icon: MessageCircle, n: '02', title: 'landing.how.step2.title', desc: 'landing.how.step2.desc', ring: 'ring-violet-500/30 bg-violet-500/10 text-violet-600' },
    { icon: Shield, n: '03', title: 'landing.how.step3.title', desc: 'landing.how.step3.desc', ring: 'ring-emerald-500/30 bg-emerald-500/10 text-emerald-600' },
  ] as const;
  ...
  return (
    <div
      className={`min-h-screen flex flex-col overflow-x-hidden ${
        isDark ? 'dark bg-[#020617] text-slate-100' : 'bg-[#F8F8F6] text-[#0F172A]'
      }`}
    >
      {/* Header */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onSignIn}
              className="hidden sm:inline-flex px-4 py-2 text-sm font-semibold text-slate-200 hover:text-[#FDBA74] transition-colors"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-full text-sm font-semibold bg-gradient-to-r from-[#F97316] via-[#FB923C] to-amber-400 text-white hover:opacity-95 shadow-lg shadow-[#F97316]/40 transition-opacity"
            >
      ...
      <section className="relative min-h-[88vh] flex items-center overflow-hidden bg-[#0F172A] text-white">
        {HERO_SLIDES.map((src, i) => (
          ...
          >
            <img
              src={src}
              alt=""
              className={`absolute inset-0 w-full h-full object-cover ${motionEnabled ? 'landing-hero-slide' : ''}`}
            />
          </div>
        ))}

        <div className="absolute inset-0 bg-gradient-to-br from-[#0F172A]/95 via-[#1e1b4b]/85 to-[#F97316]/40" />
        <div
          className={`absolute inset-0 opacity-60 mix-blend-screen bg-gradient-to-tr from-cyan-500/30 via-transparent to-fuchsia-500/30 ${
        ...
        {motionEnabled && (
          <>
            <div className="landing-orb absolute top-20 right-[10%] w-72 h-72 rounded-full bg-[#F97316]/30 blur-3xl" />
            <div className="landing-orb landing-orb-delay absolute bottom-10 left-[5%] w-96 h-96 rounded-full bg-cyan-500/20 blur-3xl" />
            <div className="landing-orb absolute top-1/2 left-1/2 -translate-x-1/2 w-64 h-64 rounded-full bg-violet-500/20 blur-3xl" />
          </>
        )}

        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-20 w-full">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-[#F97316]/20 to-cyan-500/20 border border-white/20 text-xs font-semibold uppercase tracking-wider text-[#FDBA74] mb-6">
          ...
          <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-12 items-center">
            <div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-black tracking-tight leading-[1.05] max-w-2xl">
                ...
                <span
                  className={`bg-gradient-to-r from-[#F97316] via-amber-300 to-cyan-400 bg-clip-text text-transparent ${
                    motionEnabled ? 'landing-shimmer-text' : ''
                  }`}
                >
              ...
              <div className="mt-9 flex flex-col sm:flex-row gap-3">
                <button
                  type="button"
                  onClick={() => onStart('client', 'signup')}
                  className="inline-flex items-center justify-center gap-2 px-7 py-4 rounded-2xl bg-gradient-to-r from-[#F97316] to-amber-400 text-[#0F172A] font-bold hover:scale-[1.02] active:scale-[0.98] shadow-xl shadow-[#F97316]/40 transition-transform"
                >
                  {t('landing.hero.ctaClient')}
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 px-7 py-4 rounded-2xl border-2 border-cyan-400/50 bg-cyan-500/10 text-white font-bold hover:bg-cyan-500/20 backdrop-blur-sm transition-colors"
                >
              ...
              <div className="rounded-3xl border border-white/15 bg-white/10 backdrop-blur-xl p-6 shadow-2xl shadow-black/30">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { icon: MessageCircle, label: t('auth.client.bullet3'), color: 'text-cyan-400' },
                    { icon: Shield, label: t('landing.fees.holdBody'), color: 'text-violet-400' },
            ...
            {HERO_SLIDES.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${
                  i === slideIndex ? 'w-10 bg-[#F97316]' : 'w-3 bg-white/30 hover:bg-white/50'
                }`}
              />
            ))}
          </div>
      ...
      {/* Stats — gradient band */}
      <section className="relative bg-gradient-to-r from-[#F97316] via-amber-500 to-cyan-500 text-white">
      ...
      <section id="pricing" className="scroll-mt-20 py-16 sm:py-24 bg-[#0F172A] text-white relative overflow-hidden">
        {motionEnabled && (
          <div className="landing-orb absolute -top-20 -right-20 w-96 h-96 rounded-full bg-[#F97316]/20 blur-3xl" />
        )}
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-[#F97316] to-amber-400 text-[#0F172A]">
          ...
          <div className="grid lg:grid-cols-2 gap-5 mt-10">
            ...
            <div className="p-6 rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 to-blue-500/5">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-cyan-500/20 text-cyan-300 text-xs font-bold uppercase">
          ...
          <div className="mt-8 grid lg:grid-cols-2 gap-6">
            <div className="p-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm">
              ...
              <div className="space-y-3">
                ...
                <div className="h-px bg-white/10" />
                <div className="flex justify-between items-center p-3 rounded-xl bg-gradient-to-r from-[#F97316]/20 to-amber-500/10 border border-[#F97316]/30">
                  <span className="text-sm font-bold">{t('landing.fees.exampleClientPays')}</span>
                  <span className="text-xl font-black text-[#FDBA74]">{fmt(exampleGross)}</span>
                </div>
              </div>
            </div>
              ...
              {PLANS.map((plan) => (
                <div
                  ...
                  className={`relative p-6 rounded-2xl border transition-transform hover:-translate-y-1 ${
                    plan.popular
                      ? 'border-[#F97316] bg-gradient-to-b from-[#F97316]/20 to-[#F97316]/5 shadow-lg shadow-[#F97316]/20'
                      : 'border-white/10 bg-white/5'
                  }`}
                >
                  {plan.popular && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full bg-[#F97316] text-[10px] font-bold uppercase tracking-wider">
                      {t('landing.fees.popular')}
                    </span>
                  )}
                  <p className="text-sm font-semibold text-slate-300">{t(PLAN_LABELS[plan.id])}</p>
                  <p className="mt-2 text-4xl font-black bg-gradient-to-r from-[#F97316] to-amber-300 bg-clip-text text-transparent">
                    {plan.feePercent}%
                  </p>
      ...
      {/* FAQ */}
      <section id="faq" className="scroll-mt-20 py-16 sm:py-24 bg-[#F8F8F6] dark:bg-[#020617] border-y border-slate-200/80 dark:border-slate-800">
      ...
      <section id="for-pros" className="scroll-mt-20 py-16 sm:py-24 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-[#0F172A] via-indigo-950 to-[#F97316]/30" />
        {motionEnabled && (
          <div className="landing-orb absolute bottom-0 right-0 w-[30rem] h-[30rem] rounded-full bg-fuchsia-500/15 blur-3xl" />
        )}
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 grid md:grid-cols-2 gap-6 text-white">
          <article className="p-8 rounded-3xl border border-cyan-400/30 bg-gradient-to-br from-cyan-500/15 to-blue-600/10 backdrop-blur-sm">
            ...
            <button
              type="button"
              onClick={() => onStart('client', 'signup')}
              className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-cyan-300 hover:text-white transition-colors"
            >
              {t('landing.audience.client.cta')}
              <ArrowRight className="w-4 h-4" />
            </button>
          </article>
          <article className="p-8 rounded-3xl border border-[#F97316]/40 bg-gradient-to-br from-[#F97316]/25 to-amber-500/10 backdrop-blur-sm">
            ...
            <button
              type="button"
              onClick={() => onStart('pro', 'signup')}
              className="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#F97316] to-amber-400 text-[#0F172A] text-sm font-bold hover:opacity-95 transition-opacity"
            >
      ...
      <section className="py-16 sm:py-24 relative">
        <div className="absolute inset-0 bg-gradient-to-r from-[#F97316]/10 via-violet-500/10 to-cyan-500/10" />
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              type="button"
              onClick={() => onStart('client', 'signup')}
              className="w-full sm:w-auto px-8 py-4 rounded-2xl bg-gradient-to-r from-[#F97316] via-amber-400 to-yellow-300 text-[#0F172A] font-bold hover:scale-[1.02] shadow-xl shadow-[#F97316]/30 transition-transform"
            >
            ...
            <button
              type="button"
              onClick={onSignIn}
              className="w-full sm:w-auto px-8 py-4 rounded-2xl border-2 border-slate-300 dark:border-slate-600 font-bold hover:border-[#F97316] transition-colors"
            >
          ...
          <div className="flex flex-col sm:items-center gap-4 flex-1 sm:px-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full border border-white/15 bg-white/5 text-white text-sm font-semibold hover:border-[#F97316]/60 hover:bg-[#F97316]/15 hover:text-[#FDBA74] transition-colors"
              >
                <InstagramIcon />
                Instagram
              </a>
              <a
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full border border-white/15 bg-white/5 text-white text-sm font-semibold hover:border-cyan-400/60 hover:bg-cyan-500/15 hover:text-cyan-300 transition-colors"
              >
                <TikTokIcon />
                TikTok
              </a>
